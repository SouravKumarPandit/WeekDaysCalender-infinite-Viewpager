...
/*
        if (getBeanDate()[0].get(Calendar.YEAR)!= iYear)
        {
            return getBeanDate()[0].get(Calendar.YEAR)> iYear ? 1 : -1;
        }
        else if (getBeanDate()[0].get(Calendar.MONTH)!= iMonth)
        {
            return getBeanDate()[0].get(Calendar.MONTH) > iMonth? 1 : -1;
        }
        else if (getBeanDate()[0].get(Calendar.DATE)!= iDay)
        {
            return getBeanDate()[0].get(Calendar.DATE) > iDay ? 1 : -1;
        }else
        return 1;*/
    }
